...
def process_isolated_build_config(cmdline_opts, config_opts):
    """
    ...
    if not cmdline_opts.isolated_build:
        return

    config_opts["isolated_build"] = True

    json_conf, repo_reference = cmdline_opts.isolated_build
    ...
    # With isolated build, we always assert that we are reproducing the build
    # with the same image.
