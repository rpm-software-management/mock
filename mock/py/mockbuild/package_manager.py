...
def package_manager_exists_on_host(name, config_opts):
    ...
    if name not in real_pathname:
        getLog().warning(
            "Not using '%s', it is symlink to '%s'", pathname, real_pathname
        )
        return False
    return True
...
in Mock config.""".format(
                        desired.upper(), manager.upper()
                    )
                )
...
def package_manager(config_opts, buildroot, plugins, bootstrap_buildroot=None):
    ...
    return cls(config_opts, buildroot, plugins, bootstrap_buildroot, is_bootstrap_image)
...
class _PackageManager(object):
    ...
    @traceLog()
    def __init__(
        self, config, buildroot, plugins, bootstrap_buildroot, is_bootstrap_image
    ):
        self.config = config
...
Error:      Neither dnf-utils nor yum-utils are installed. Dnf-utils or yum-utils are needed to complete this action.
    ...
    def _check_command(self):
        ...
        if not os.path.exists(self.command):
            raise Exception(
                """Command {0} is not available. Either install package containing this command
or run mock with --yum or --dnf to overwrite config value. However this may
lead to different dependency solving!""".format(
                    self.command
                )
            )
...
class Yum(_PackageManager):
    ...
    def __init__(
        ...
        super(Yum, self).__init__(
            config, buildroot, plugins, bootstrap_buildroot, is_bootstrap_image
        )
        ...
        if bootstrap_buildroot is not None:
            ...
            yum_deprecated_path = bootstrap_buildroot.make_chroot_path(
                yum_deprecated_path
            )
            yum_builddep_deprecated_path = bootstrap_buildroot.make_chroot_path(
                yum_builddep_deprecated_path
            )
        else:
            ...
            if os.path.exists(yum_deprecated_path):
                ...
                self.resolvedep_command = [
                    ...
                    "--requires",
                    "--config",
                    self.buildroot.make_chroot_path("etc", "yum", "yum.conf"),
                ]
    ...
    def initialize_config(self):
        ...
        if self.config["subscription-manager.conf"]:
            ...
            shutil.copy(
                "/etc/rhsm/rhsm.conf", self.buildroot.make_chroot_path("etc", "rhsm")
            )
    ...
    def install(self, *pkgs, **kwargs):
        ...
        if check:
            out = self.execute(
                "resolvedep", *pkgs, returnOutput=True, printOutput=False, pty=False
            )
            _check_missing(out)
...
class Dnf(_PackageManager):
    ...
    def build_invocation(self, *args):
        ...
        if (
            "forcearch" in self.config
            and "--forcearch" not in self.config["dnf_common_opts"]
            and self.config["forcearch"]
        ):
            self.config["dnf_common_opts"].extend(
                ["--forcearch", self.config["forcearch"]]
            )
