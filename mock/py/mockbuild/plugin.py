            ...
            if self.plugin_conf.get("{0}_enable".format(plugin)):
                try:
                    ...
                    buildroot.root_log.warning(
                        "{0} plugin is enabled in configuration but is not installed".format(
                            plugin
                        )
                    )
                    continue
                ...
                if not hasattr(module, "requires_api_version"):
                    raise Error(
                        'Plugin "%s" doesn\'t specify required API version' % plugin
                    )
                requested_api_version = module.requires_api_version
                if requested_api_version != current_api_version:
                    raise Error(
                        "Plugin version mismatch - requested = %s, current = %s"
                        % (requested_api_version, current_api_version)
                    )

                module.init(
                    self, self.plugin_conf["{0}_opts".format(plugin)], buildroot
                )
