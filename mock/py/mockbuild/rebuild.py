...
@traceLog()
def rebuild_generic(
    items, commands, buildroot, config_opts, cmd, post=None, clean=True
):
    start = time.time()
    try:
        for item in items:
            ...
            log.info(
                "Done(%s) Config(%s) %d minutes %d seconds",
                item,
        ...
        elapsed = time.time() - start
        log.error(
            "Exception(%s) Config(%s) %d minutes %d seconds",
            item,
            buildroot.shared_root_name,
            elapsed // 60,
            elapsed % 60,
        )
        log.info("Results and/or logs in: %s", buildroot.resultdir)
...
def do_rebuild(config_opts, commands, buildroot, options, srpms):
    ...
    rebuild_generic(
        srpms, commands, buildroot, config_opts, cmd=build, post=post_build, clean=clean
    )
...
def do_buildsrpm(config_opts, commands, buildroot, options, args):
    ...
    if (
        options.sources
        and not os.path.isdir(options.sources)
        and not os.path.isfile(options.sources)
    ):
        raise BadCmdline(
            "Input sources directory or file does not exist: %s" % options.sources
        )
    clean = config_opts["clean"]

    def cmd(spec):
        return commands.buildsrpm(
            spec=spec,
