...
def lvm_do(*args, **kwargs):
    env = os.environ.copy()
    env["LC_ALL"] = "C.UTF-8"
    output = util.do(*args, returnOutput=True, env=env, returnStderr=False, **kwargs)
    return output
...
class Lock(object):
    ...
    def lock(self, exclusive, block=False):
        ...
        try:
            fcntl.lockf(
                self.lock_file.fileno(), lock_type | (0 if block else fcntl.LOCK_NB)
            )
        except IOError as e:
...
class LvmPlugin(object):
    ...
    def create_lock(self, name):
        return Lock(
            self.basepath, "{0}-{1}".format(name, self.conf_id), self.sleep_time
        )
    ...
    def query_lv(self, lv_name, *options):
        query = [
            entry[1:]
            for entry in self.query_lvs("lv_name", *options)
            if entry[0] == lv_name
        ]
        if query:
            return query[0]
    ...
    def _lv_entry_is_our(self, entry):
        ...
        return (
            attrs[0] == "V"
            and pool_lv == self.pool_name
            and name.replace("+", "").startswith(self.prefix_name())
        )
    ...
    def list_our_lvs(self, *options):
        ...
        return [
            [entry[0]] + entry[3:] for entry in lvs if self._lv_entry_is_our(entry[:3])
        ]
    ...
    def set_current_snapshot(self, name):
        if not self.lv_is_our(name):
            raise LvmError(
                "Snapshot {0} doesn't exist".format(self.remove_prefix(name))
            )
    ...
    def allocated_pool_data(self):
        ...
        compiled_re = re.compile(
            r".*Allocated pool data\s*([0-9.]*)%$.*", re.DOTALL | re.MULTILINE
        )
    ...
    def allocated_pool_metadata(self):
        ...
        output = lvm_do(["lvdisplay", pool_id])
        compiled_re = re.compile(
            r".*Allocated metadata\s*([0-9.]*)%$.*", re.DOTALL | re.MULTILINE
        )
        r_match = compiled_re.match(output)
        if r_match:
            return float(r_match.group(1))
    ...
    def prepare_mount(self):
        ...
        if lv_path:
            ...
            self.mount = mounts.FileSystemMountPoint(
                self.root_path, self.fs_type, lv_path, options=mount_options
            )
        ...
        for i, mount in enumerate(self.buildroot.mounts.managed_mounts):
            if mount.mountsource == self.root_path:
                newmount = mounts.FileSystemMountPoint(
                    mount.mountpath, self.fs_type, lv_path, options=mount_options
                )
    ...
    def check_pool_size(self):
        ...
        self.buildroot.root_log.info(
            "LVM plugin enabled. Allocated pool data: {0}%. Allocated metadata: {1}%.".format(
                size_data, size_metadata
            )
        )
        if (
            "check_size" not in self.lvm_conf
            or ("check_size" in self.lvm_conf and self.lvm_conf["check_size"])
        ) and (
            (size_metadata and size_metadata > 90) or (size_data and size_data > 90)
        ):
            raise LvmError(
                "Thin pool {0}/{1} is over 90%. Please enlarge it.".format(
                    self.vg_name, self.pool_name
                )
            )
    ...
    def hook_scrub(self, what):
        ...
        remaining = [
            name
            for name, attr, pool_lv in self.query_lvs("lv_name", "lv_attr", "pool_lv")
