...
"""Providing a pytests for file_util module"""
...
def set_immutable(path: Path, immutable: bool):
    ...
    return subprocess.run(["chattr", flag, str(path)],
                          check=True,
...
def chattr_works_or_skip(path: Path):
    """sanity check of chattr actually runs or pytest.skip"""
    if not hasattr(chattr_works_or_skip, "has_chattr"):
        chattr_works_or_skip.has_chattr = shutil.which("chattr") is not None
...
class TestRmtree:
    ...
    def test_rmtree_immutable_file_with_selinux(self, temp_dir):
    ...
    def test_rmtree_immutable_file_without_selinux(self, temp_dir):
